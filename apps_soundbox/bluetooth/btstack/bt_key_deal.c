	...
	int msg_echo_start = MSG_ECHO_START;
        ...
        switch(status)
        {
            ...
            case BT_STATUS_PHONE_INCOME:
				...
				#if IPOHONE_RING_EN
				if(!check_support_feature_ag(0x0008))
				{
                	user_ctrl_prompt_tone_play(BT_STATUS_PHONE_INCOME,NULL);
				}
				#else
	...
	switch(msg[0]){
        ...
        case MSG_BT_PREV_FILE:
         ...
         case MSG_BT_MUSIC_EQ:
		 	/*
		 	bt_eq_flag=!bt_eq_flag;
			if(bt_eq_flag)
			{
			    user_val->bt_eq_mode=3;
				set_bt_eq_mode(user_val->bt_eq_mode);
            	printf("MSG_BT_EQ %d\n", user_val->bt_eq_mode);
			}
			else
			{
		...
		case MSG_VOL_MAX:
			volmax_end_f=1;
			pa_umute();
			bt_prompt_play_by_name(BPF_VOLMAX_MP3, NULL);
			break;
		case MSG_VOL_MIN:
			volmmin_end_f=1;
			pa_umute();
			bt_prompt_play_by_name(BPF_VOLMIN_MP3, NULL);
			break;
...
//            puts("power_off_tone_end\n");
        ...
        case MSG_HALF_SECOND:
            ...
            if((BT_STATUS_CONNECTING == get_bt_connect_status())   ||
               ...
               (BT_STATUS_PLAYING_MUSIC == get_bt_connect_status()))/*连接状态*/
            {
                if(get_dac_energy_value() <= 0x00000050)
				 {
					 led_fre_set(0,0);							/*暂停常亮*/
					 if((get_call_status() == BT_CALL_INCOMING)||(get_call_status() == BT_CALL_OUTGOING)||(get_call_status() == BT_CALL_ACTIVE))					
                     {
                        pa_umute();
                     }
					 else 
                ...
                if(user_val->auto_shutdown_cnt==0)
                {
					...
					going_to_pwr_off = 1;
					poweroff_cnt=20;
					os_taskq_post("MainTask", 1, MSG_GOTO_IDLE);
