...
static void TaskMain(void *p)
{
   ...
   // led_init();
	...
	JL_SYSTEM->LVD_CON |= BIT(0); // 复位电压2.2v
...
#if ((POWERON_MOS_EN) || (POWERON_PR2))
			if(first_power_key==1)break;
#endif
#if MUSIC_U_SD_SWITCH
...
//            Post_msg_to_rcsp_Task(MSG_RSCP_CHANGE_WORDMODE_ACTION,0,0);
        ...
        case MSG_VOL_DOWN:
#if VOLMAX_SET
			if(volmax_end_f==1)//播放最大提示音的时候无作用
break;
			#endif
			...
			if(dac_ctl.sys_vol_l==0)
			{
				if(volmmin_end_f==0)
				 { 
					os_taskq_post(keymsg_task_name, 1, MSG_VOL_MIN);
				 }
			}
#endif
            break;
             ...
             if(dac_ctl.sys_vol_l == MAX_SYS_VOL_L)
			  {
					...
					if(volmax_end_f==0)
					{ 
						os_taskq_post(keymsg_task_name, 1, MSG_VOL_MAX);
					}
#else
					send_key_voice(500);
#endif
