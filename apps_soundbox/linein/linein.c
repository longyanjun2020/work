...
#if AUX_ENERGY_EN
/*
ENC_LINE_LR_CHANNEL 根据 LINEIN_CHANNEL 宏所选输入的通道确定，
如果是 LINEIN_CHANNEL 定义为 DAC_AMUX_DAC_L，则传入为
ladc_reg_init(ENC_DAC_LEFT_CHANNEL,LADC_SR16000);
*/
    ...
    while (1)
    {
		...
		switch (msg[0])
        {
			...
			case MSG_VOL_MAX:
				...
				aux_dac_channel_off();
				pa_umute();
				linein_prompt_play(BPF_VOLMAX_MP3);
				break; 
			case MSG_VOL_MIN:
				puts("msg_bt_volmax\n");
				pa_umute();
				aux_dac_channel_off();
				linein_prompt_play(BPF_VOLMIN_MP3);
				break; 
