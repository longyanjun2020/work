...
u8 poweroff_cnt=0;
u8 first_power_key=1;
extern bool bt_rejiect_flag;
bool volmax_end_f=0;
bool volmmin_end_f=0;
u8 vm_work_mode=VM_BT_MODE;
...
void key_scan(void *ptr)
{
    ...
    {
        if(((back_last_key==9)||(back_last_key==8)||(back_last_key==7))&&(key_status==KEY_SHORT_UP))
        {
            if(key_press_flag == 0)
                key_press_flag = 1;
